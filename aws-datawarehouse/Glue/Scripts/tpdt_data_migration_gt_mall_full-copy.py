...
mall = spark.sql("""
    SELECT
        ...
        t2.DESCRIPTION_ENG as desc_en,
        t2.DESCRIPTION_TCH as desc_tc,
        t2.DESCRIPTION_SCH as desc_sc,
        t2.LOC_LONG_EN as address_en,
        t2.LOC_LONG_TC as address_tc,
        t2.LOC_LONG_SC as address_sc,
        case when t2.DISTRICT is null then null
        when t2.DISTRICT = 0 then 'HONG_KONG_ISLAND'
        when t2.DISTRICT = 1 then 'KOWLOON'
        else 'NEW_TERRORITIES' end as district,
        loc_short_en as location_en,
        loc_short_tc as location_tc,
        loc_short_sc as location_sc,
        t2.open_hour_en as opening_hour_en,
        t2.open_hour_tc as opening_hour_tc,
        t2.open_hour_sc as opening_hour_sc,
        t1.phone,
        t2.LOC_LATITUDE as latitude,
        t2.LOC_LONGITUDE as longitude,
        case when left(t2.WEBSITE_URL,3) = 'www' then concat('http://', t2.WEBSITE_URL) else facebook_url end as WEBSITE_URL,
        t2.email,
        case when services_webview_eng is not null then concat('https://uat-shkpmalls.shkp.com',  services_webview_eng) else null end as service_website_url_en,
        case when services_webview_tch is not null then concat('https://uat-shkpmalls.shkp.com',  services_webview_tch) else null end as service_website_url_tc,
        case when services_webview_sch is not null then concat('https://uat-shkpmalls.shkp.com',  services_webview_sch) else null end as service_website_url_sc,
...
phase_mall_link = spark.sql("""Select phase_id, t1.mall_id as mall_id from phase p left join vip_mall t1 on p.mall = t1.cms_mall_id
 left join cms_mall t2 on t2.id = t1.cms_mall_id
where p.status = 1 and (t1.mall_id is not null or p.mall_id in (45,54)) and t1.mall_id != 1002""")
...
on case when t1.partner_id = 5001 then 1
...
case when t2.phase_lang1 is null then then (case when t4.mall_name_en is not null then t4.mall_name_en else t3.name_lang1 end) else t2.phase_lang1 end as location_en,
case when t2.phase_lang2 is null then then (case when t4.mall_name_en is not null then t4.mall_name_tc else t3.name_lang2 end) else t2.phase_lang2 end  as location_tc,
case when t2.phase_lang3 is null then then (case when t4.mall_name_en is not null then t4.mall_name_sc else t3.name_lang3 end) else t2.phase_lang3 end  as location_sc,
t1.open_hour_en as opening_hour_en,
t1.open_hour_tc as opening_hour_tc,
t1.open_hour_sc as opening_hour_sc,
t1.phone as phone,
case when t4.partner_id = 5003 then t4.partner_shop_id else null end as shop_code_for_carpark,
case when t1.ticket_type = 1 then 'eatEasy'
when t1.ticket_type = 2 then 'maxims'
...
case when t2.visible = 'V' then 1 else 0 end as display_in_directory,
...
case when t5.partner_id = 5007 then t5.partner_shop_id else null end as merchant_app_shop_code,
...
t2.phase_lang1 as location_en,
t2.phase_lang2 as location_tc,
t2.phase_lang3 as location_sc,
'' as opening_hour_en,
'' as opening_hour_tc,
'' as opening_hour_sc,
'' as phone,
