...
receipt_df = t_receipt.toDF().withColumn('PatitionKey', to_date(col('updated_date')))

print("Saving Result into target destination...")
receipt_output_path="s3://tpdt-staging/receipt/"
receipt_df.write.format('parquet').mode('overwrite').partitionBy("PatitionKey").option("header",True).save(receipt_output_path)
