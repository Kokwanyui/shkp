...
vehicles = spark.sql("""
Select lpn,
case when validationSts = 1 then 'TRUE' else 'FALSE' end as validationSts,
memberId
from vehicles
""")
preaction = "Delete from public.vehicles;"

glueContext.write_dynamic_frame.from_catalog(
    frame = DynamicFrame.fromDF(vehicles, glueContext, "full_load"),
    database= "tpdt_03redshift",
    table_name = "data_migration_public_vehicles",
    catalog_connection="tpdt-03redshift-connector",
    redshift_tmp_dir="s3://tpdt-athena-queryresult",
    transformation_ctx = "data_migration_public_vehicles",
    additional_options={"preactions": preaction}
    )
