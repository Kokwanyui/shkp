...
bp_transaction_df = bp_transaction_df.filter('date(created_date) between to_date("2022-02-01") and to_date("2022-12-31")')
...
spending_transaction_df = spending_transaction_df.filter('date(created_date) between to_date("2022-02-01") and to_date("2022-12-31")')
...
receipt_df = receipt_df.filter('date(created_date) between to_date("2022-02-01") and to_date("2022-12-31")')
...
partner_spending_transaction_df = partner_spending_transaction_df.filter('date(created_date) between to_date("2022-02-01") and to_date("2022-12-31")')
...
case when receipt_datetime is not null then date_format(receipt.receipt_datetime,"yyyy-MM-dd'T'HH:mm:ss'.000Z'") else date_format(receipt.receipt_date,"yyyy-MM-dd'T'HH:mm:ss'.000Z'") end as LEGACY_Activity_Date_Time__c,
...
date_format(tranx.approval_date, "yyyy-MM-dd'T'HH:mm:ss'.000Z'") as LEGACY_Approval_Date__c,
...
date_format(receipt.created_date, "yyyy-MM-dd'T'HH:mm:ss'.000Z'") as LEGACY_Created_Date__c,
date_format(receipt.updated_date, "yyyy-MM-dd'T'HH:mm:ss'.000Z'") as LEGACY_Updated_Date__c,
